...
Return ONLY raw JSON (no markdown fences, no extra text):
{
  ...
  try {
    ...
    try {
      ...
      // Sometimes Gemini returns extra text; try to extract the JSON object
      const jsonMatch = cleaned.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        parsed = JSON.parse(jsonMatch[0]);
      } else {
        console.error('[gemini] Could not parse JSON from response:', cleaned.slice(0, 200));
        return fallbackResult('Could not parse AI response.');
      }
    }

    // Normalise + validate every field so nothing downstream gets NaN
    return {
      songTitle: parsed.songTitle || null,
      confirmedArtist: parsed.confirmedArtist || null,
      confidence: clampInt(parsed.confidence, 0, 100),
      lyricMatch: Boolean(parsed.lyricMatch),
      wordMatch: Boolean(parsed.wordMatch),
      reasoning: typeof parsed.reasoning === 'string' ? parsed.reasoning : 'Analysis complete.',
    };
  } catch (err) {
    console.error('[gemini] API error:', err?.message || err);
    return fallbackResult('AI service temporarily unavailable. Please try again.');
  }
}

// Ensure a value is a finite integer within [min, max], defaulting to 0
...
function fallbackResult(reasoning = 'Analysis unavailable right now.') {
  return {
    songTitle: null,
